...
if __name__ == '__main__':
    ...
    static = (lambda x: x[:, :, 0] * 0.114 + x[:, :, 1] * 0.587 + x[:, :, 2] * 0.229)(arr)
    static = Canny(GaussianBlur(static, 1, 1), 50, 150)
    plt.imshow((lambda x: Image.fromarray(x))(static))
    plt.show()
    
    static = Canny(static)
    GaussianBlur.get(static, 1, 1)
    CannyModel()
"""
