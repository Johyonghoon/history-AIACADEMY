...
class TokenizersTest:
    ...
    def exec(self):
        ...
        urllib.request.urlretrieve("https://raw.githubusercontent.com/e9t/nsmc/master/ratings.txt",
                                   filename=os.path.join(dir_path("tests"), "data", "ratings.txt"))
        naver_df = pd.read_table(os.path.join(dir_path("tests"), "data", "ratings.txt"))
        naver_df = naver_df.dropna(how='any')
        with open(os.path.join(dir_path("tests"), "data", "naver_review.txt"), 'w', encoding='utf8') as f:
        ...
        data_file = os.path.join(dir_path("tests"), "data", "naver_review.txt")
        vocab_size = 30000
        limit_alphabet = 6000
        min_frequency = 5
        # 버트워드피스토크나이저를 설정합니다.
        ...
        # vocab 저장
        tokenizer.save_model(os.path.join(dir_path("tests"), "save"))
        # vocab 로드
        df = pd.read_fwf(os.path.join(dir_path("tests"), "save", "vocab.txt"), header=None)
        print(" ********* vocab.txt ********* ")
        print(df)
        print(" *************************** ")
        """
        ...
        tokenizer.train(os.path.join(dir_path("tests"), "data", "naver_review.txt"), vocab_size=10000, min_frequency=5)
