...
from app.configs.secrets import KOGPT_API_KEY

api = KoGPT(service_key=KOGPT_API_KEY)

# 소스코드 출처 : "https://wooiljeong.github.io/python/pykakao-kogpt/"
...
class KakaoGpt3:
    ...
    def next_sentence(self):
        ...
        result = api.generate(prompt, max_tokens=64, temperature=0.7, top_p=0.8)
        print(result['generations'][0]['text'])

    def classify_sentence(self, data):

        prompt = f"""{data}="""

        result = api.generate(prompt, max_tokens=1, temperature=0.4)
        print(result['generations'][0]['text'])
    ...
    def news_summary(self):
        ...
        result = api.generate(prompt, max_tokens=128, top_p=0.7)
        print(result['generations'][0]['text'])
    ...
    def answer_the_question(self):
        ...
        result = api.generate(prompt, max_tokens=128, temperature=0.2)
        print(result['generations'][0]['text'])
    ...
    def quiz(self):
        ...
        답:'''
        result = api.generate(prompt, max_tokens=1, temperature=0.3)
        print(result['generations'][0]['text'])

    def polite_word(self):
        ...
        result = api.generate(prompt, max_tokens=10, temperature=0.7)
        print(result['generations'][0]['text'])

    def chatting(self, request_data):
        ...
        result = api.generate(prompt, max_tokens=32, temperature=0.3, top_p=0.85)
        return result['generations'][0]['text'].split('\n')[0]
